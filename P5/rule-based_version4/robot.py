...
class Robot(object):
    ...
    def next_move(self, sensors):
        ...
        print self.location, self.maze_grid[tuple(self.location)],rotation,movement,

        self.move(rotation,movement)
        print self.heading,self.location
        # x = raw_input('x')
    ...
    def chooseroute(self,sensors):
        ...
        if openning_num == 1:
            ...
            suggested_dir = self.desired_dir()
            rotation, movement = self.choosedirection(suggested_dir)
        else:
            print current_loc
            print self.location
            x = raw_input('???')
        ...
        if movement >0:
            ...
            suggested_dir = self.desired_dir()
            if self.dir_ROT[self.heading][rotation] in suggested_dir:
                movement = max(range(1,far_mov+1))
        return rotation, movement

    def choosedirection(self,suggested_dir):
        rot = 0
        mov = 0
